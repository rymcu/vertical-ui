...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData() {
                let _ts = this;
                _ts.axios.get('/user-info/detail/' + _ts.$store.state.idUser).then(function (res) {
                    if (res) {
                        if (res.message) {
            ...
            checkNickname() {
                let _ts = this;
                _ts.axios.get('/user-info/check-nickname', {
                    params: {
            ...
            updateUser(user) {
                ...
                _ts.$refs['user'].validate((valid) => {
                    if (valid) {
                        _ts.axios.patch('/user-info/update', user).then(function (res) {
                            if (res) {
                                if (res.message) {
                                    _ts.$message.error(res.message);
                                } else {
