<template>
    <el-row class="wrapper verify" style="background-color: #F6F7F8;padding-top: 10%;">
        ...
        <el-col :xs="24" :sm="12" :xl="12" class="intro vditor-reset verify__sign">
            <div>
                <h2>欢迎来到 RYMCU</h2>
                <p><a href="/">RYMCU</a> 是一个嵌入式知识学习、交流、分享平台，专注于单片机学习。</p>
                <p>我们致力于构建一个即严谨又活泼、专业又不失有趣，为数百万人服务的开源社区。大家在这里相互<strong>信任</strong>，以<em>平等 • 自由 • 奔放</em>的价值观进行分享交流。最终，希望大家能够找到与自己志同道合的伙伴，共同成长。</p>
    ...
    export default {
        ...
        methods: {
            sendCode() {
                ...
                _ts.axios.get('/console/get-email-code', {
                    params: data
                }).then(function (res) {
                    if (res) {
            ...
            register() {
                ...
                _ts.$refs.user.validate((valid) => {
                    if (valid) {
                        ...
                        _ts.axios.post('/console/register', data).then(function (res) {
                            if (res) {
                                _ts.$message(res.message);
